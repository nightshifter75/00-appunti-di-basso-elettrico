...
Non possiamo continuare a suonare senza sapere un accidenti di niente sulla musica. Dobbiamo proseguire sapendo almeno le basi. Quindi bambini adesso cercheremo di imparare tutte le paroline che ci serviranno per affrontare seriamente lo studio della teoria musicale, che altri non è che la disciplina che ci aiuta a scrivere, leggere e capire come si *muove* la musica. 

Le note sono suoni, e il suono di per sé ha sfumature infinite (come il colore). Consideriamo le quattro caratteristiche fondamentali del suono: **timbro**, **altezza**, **durata** e **intensità**. 
...
Le altre caratteristiche (**altezza**, **durata** e **intensità**) si possono descrivere per poter poi essere rilette e riprodotte. Nei secoli ci sono stati diversi metodi per scrivere la musica, e un paio di questi sono arrivati fino a noi: il **pentagramma** e l'**intavolatura**.

Il **pentagramma** è la notazione musicale per eccellenza, valida per tutti gli strumenti musicali come ad esempio la voce. Si tratta di cinque linee chiamate **righi** (non righe, righi) separati da quattro **spazi**. Una nota viene rappresentata con una testa, un gambo e una o più code che ne determinano la durata. L'altezza della nota è invece determinata dal rigo o spazio sul quale poggia. L'altezza può essere modificata dagli accidenti e quindi parliamo di **alterazione**. Ogni alterazione è scritta a sinistra di ogni nota. L'intensità la vedremo dopo.
